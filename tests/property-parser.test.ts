...
describe("PropertyParser", () => {
  test.each(testPresets)("$name", (testPreset) => {
    const result = new PropertyParser(testPreset.environment).getValue(
      testPreset.property as any
    );
    typeof result === "object"
      ? expect(result).toMatchObject(testPreset.expectedResult)
      : expect(result).toBe(testPreset.expectedResult);
  });
});
